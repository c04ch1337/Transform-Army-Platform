...
async def get_db(request: Request) -> AsyncSession:
    """
    Get database session with RLS context automatically set.
    
    This wrapper around the base get_db() function automatically sets the
    tenant context for Row-Level Security if a tenant is authenticated.
    The tenant_id is extracted from request.state (set by TenantMiddleware).
    
    Args:
        request: FastAPI request object with tenant info in state
    ...
    Example:
        ...
        async def get_contacts(
            ...
            # RLS context is automatically set based on authenticated tenant
            # All queries are filtered by tenant_id
            result = await db.execute(select(Contact))
            return result.scalars().all()
    """
    async for session in _get_db():
        # Set RLS context if tenant is authenticated
        if hasattr(request.state, "rls_tenant_id") and request.state.rls_tenant_id:
            try:
                await set_tenant_context(session, request.state.rls_tenant_id)
                logger.debug(f"RLS context set for tenant {request.state.rls_tenant_id}")
            except Exception as e:
                logger.error(f"Failed to set RLS context: {e}")
                # For security, fail closed - don't allow queries without RLS
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to establish secure database context"
                )
...
async def get_crm_provider(
    ...
    if settings.environment == "development" and "crm" not in tenant_config.get("provider_configs", {}):
        # Initialize provider_configs if not present
...
async def get_helpdesk_provider(
    ...
    if settings.environment == "development" and "helpdesk" not in tenant_config.get("provider_configs", {}):
        ...
        # MERGE config instead of overwriting
        tenant_config["provider_configs"]["helpdesk"] = {
            "provider": "zendesk",
...
async def get_calendar_provider(
    ...
    if settings.environment == "development" and "calendar" not in tenant_config.get("provider_configs", {}):
        ...
        # MERGE config instead of overwriting
        tenant_config["provider_configs"]["calendar"] = {
            "provider": "google",
...
async def get_cache() -> CacheService:
    ...
    Example:
        @app.get("/api/data")
        async def get_data(cache: CacheService = Depends(get_cache)):
            data = await cache.get_or_fetch("key", fetch_func)
            return data
    """
...
async def get_job_queue(queue_name: str = "default") -> JobQueue:
    ...
    queue = JobQueue(queue_name=queue_name, redis_client=redis_client)
